...
## 🚨 지도가 회색·단색 화면으로만 보인다면 (가장 흔한 사고)

**증상**: 앱은 열리고 검색도 되는데, 지도 자리에 아무 그림 없이 **단색 배경만** 보입니다.

**원인**: Mapbox 토큰의 **URL 제한에 경로가 들어가 있습니다.**
`geo-2026.github.io/travel-internship/` 처럼 적으면 **지도 타일만** 403 으로 막힙니다.
타일은 브라우저의 **웹 워커**가 받아 오는데, 워커 요청에는 경로가 빠진
`https://geo-2026.github.io/` 만 실려 나가기 때문입니다. 스타일·검색·경로는
메인 스레드라 정상 동작해서, 겉으로는 "지도만 고장 난" 것처럼 보입니다.

**조치 (1분)**

1. [Mapbox 대시보드 → Account → Tokens](https://account.mapbox.com/access-tokens/) 접속
2. `travel-internship-class` 토큰 열기
3. **URL restrictions** 를 아래 **한 줄로** 고칩니다 — 경로 없이 도메인만

   ```
   geo-2026.github.io
   ```

4. 저장 후 배포 주소를 **새로고침**(캐시 무시: `Ctrl+Shift+R`)

> Mapbox 의 URL 제한은 **와일드카드(`*`)를 지원하지 않습니다.** 도메인만 적으면
> 그 아래 경로는 자동으로 허용되므로 `/travel-internship/` 도 그대로 동작합니다.
> 도메인까지만 제한해도 방어선은 그대로입니다. `geo-2026.github.io` 는 교사 계정의
> GitHub Pages 도메인이라 외부인이 이 주소로 페이지를 올릴 수 없습니다.

앱은 이 상황을 스스로 감지해 **지도 위에 원인과 조치 방법을 띄웁니다.**
배너가 보이면 위 절차대로 고치면 됩니다.
...
5. **URL restrictions** 에 아래 **한 줄만** 입력합니다.
   ...
   > **와일드카드(`*`)는 지원되지 않습니다.** 도메인만 적으면 그 아래 경로는 자동으로
   > 허용되므로 `/travel-internship/` 도 그대로 동작합니다.
   >
   ...
   실제로 겪은 문제입니다. `geo-2026.github.io/travel-internship/` 처럼
   **경로까지** 적으면 이렇게 됩니다.
...
**이미 만들어져 있고, 담고 있는 주소가 배포 주소와 같은지 확인했습니다.**
주소를 바꾸지 않는 한 다시 만들 필요가 없습니다.

### 학생용 사용 안내서 — `docs/guide.html`

학생에게 나눠 줄 **A4 두 쪽(양면 한 장)** 짜리 안내서입니다. QR·앱 주소·3단계 사용법·
검색 요령(현지어)·문제 해결표가 들어 있습니다.

- **인쇄**: 브라우저로 `docs/guide.html` 을 열고 `Ctrl+P` → 용지 A4 → **배경 그래픽 켜기**.
  여백은 기본값 그대로 두면 두 쪽에 맞습니다.
- **웹으로 보여 주기**: 배포 후 <https://geo-2026.github.io/travel-internship/docs/guide.html>
- 외부 글꼴·스크립트를 쓰지 않아 학교망에서도 그대로 열립니다. QR 은 같은 폴더의
  `qr.png` 를 참조하므로 두 파일을 함께 두세요.
